...
— Root cause: the model.json committed under `frontend/public/models/lstm/`
was generated by Keras 3.14, which writes `batch_shape` (not
`batch_input_shape`), `DTypePolicy` dtype objects, and `__keras_tensor__`-
wrapped `inbound_nodes`. TF.js 4.22 only understands the legacy Keras 2
format and rejects the Keras 3 output at load time.

- `backend/scripts/train_lstm.py`:
  - Forces legacy Keras 2 via `TF_USE_LEGACY_KERAS=1` env var (set
    *before* `import tensorflow`) and switches to the `tf_keras` package.
  - Added `_validate_topology_for_tfjs` that rejects exports containing
    Keras 3-specific fields (`batch_shape`, `DTypePolicy`). Fails loudly
    at export time so we never ship a broken model again.
  - Strips the `:0` tensor suffix from weight names in the manifest (TF.js
    wants `layer/kernel`, not `layer/kernel:0`).
  - README note: `pip install tf_keras` is now required.

- **User must re-run training once** to regenerate `model.json` and
  `group1-shard1of1.bin` under `frontend/public/models/lstm/`, then commit
  and redeploy.

**Vercel SPA rewrite regex didn't reliably exclude static assets.** The
previous pattern used a negative lookahead `(?!models/|assets/|.*\.…)`
which path-to-regexp may or may not preserve through its compilation.
Replaced with the canonical SPA pattern:

    "source": "/:slug([^.]*)", "destination": "/index.html"

This rewrites only **extensionless** paths (SPA routes like `/`, `/about`),
leaving all static file paths (with dots: `.onnx`, `.json`, `.bin`, `.js`,
`.css`, …) untouched — they're served from `public/` as-is.

**XGBoost "waiting…" guard.** Defensive paths added in
`frontend/src/hooks/useForecast.js`:
- If the server returns an `xgboost` result but `metadata.xgb_features` is
  missing, immediately set an explicit error instead of pushing an inference
  task that would hang.
- If no `xgboost` result has `metadata.client_side === true`, surface
  "server did not flag client_side inference" on the card rather than
  leaving it stuck at "waiting…".
- Browser inference `.catch` now `console.warn`s the real error with a
  `XGBoost:` prefix so the console tells us which step failed.

**Composite/ensemble clarification.** The "composite model that trains on
top of all 6 model outputs" is not a trained stacking meta-learner — it's
a **regime-conditional weighted ensemble** (`frontend/src/ml/MetaEnsemble.js`).
The HMM classifies the current regime (bull/bear × low/high vol) from the
VIX percentile rank and the HMM state posterior, and `MetaEnsemble` combines
the 6 model medians with regime-specific weights derived in
`backend/app/services/meta_learner.py::get_ensemble_weights`. It renders
below the 2×3 method-card grid as `EnsembleCard` + `KellyPanel` +
`ScenarioPanel`. The ENSEMBLE WEIGHT column at the top is `—` when
`regime_probs` didn't come back from the HMM (a failed HMM propagates to
zero ensemble weights).
...
1. `cd backend && pip install tf_keras && python scripts/train_lstm.py`
2. `git add frontend/public/models/lstm/ && git commit -m "Retrain LSTM with Keras 2 export" && git push`
