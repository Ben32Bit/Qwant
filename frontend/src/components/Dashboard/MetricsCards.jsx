  ...
  return (
    <div className="flex items-center justify-between py-1"
      ...
      <div className="text-right">
        ...
        {sub && <span className="mono text-xs ml-1.5" style={{ color: 'var(--text-secondary)' }}>{sub}</span>}
      </div>
    </div>
  )
}
...
function SectionTitle({ children }) {
  return (
    <div className="mono text-xs font-bold uppercase tracking-widest pt-2.5 pb-1"
...
export default function MetricsCards({ metrics, loading }) {
  ...
  return (
    <div className="rounded-lg border p-3" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
      <div className="grid gap-x-5" style={{ gridTemplateColumns: hasBenchmark ? '1fr 1fr 1fr' : '1fr 1fr' }}>
        ...
        <div>
          ...
          {m.deflated_sharpe != null && (
            <Row
              label="Deflated SR"
              value={`${(m.deflated_sharpe * 100).toFixed(1)}%`}
              valueClass={dsrClass(m.deflated_sharpe)}
              sub={m.deflated_sharpe >= 0.95 ? '✓ low overfit' : m.deflated_sharpe >= 0.90 ? '⚠ moderate' : '✗ overfit?'}
            />
          )}
          {m.skewness != null && (
          ...
          {m.excess_kurtosis != null && (
            <Row label="Kurtosis" value={fmtRatio(m.excess_kurtosis)} />
          )}
        </div>

        {/* Col 3 — vs Benchmark (only when benchmark data present) */}
        {hasBenchmark && (
          <div>
            ...
            <Row label="Track. Error"  value={fmtPct(m.tracking_error)} />
